...
- callType: classify what kind of statement this is. This single field decides whether the take is scored — there is no separate flag.
  ...
  - "pair_trade" — one leg of a paired long/short ("long X / short Y", "own X over Y"). Emit each leg as its OWN take carrying its OWN direction: the long leg's stance is "bull" and the short leg's stance is "bear" — never "mixed" (the pair is mixed; each individual leg is not).

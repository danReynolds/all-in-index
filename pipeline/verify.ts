...
A thesis's published quote must, on its own, prove its stance AND be word-for-word real. So for every thesis decide:

- "keep": the current quote (a) already contains an explicit, company-specific economic claim matching the bull/bear/mixed stance (or the stance is neutral/mixed and the quote is a genuine company view), AND (b) is an EXACT verbatim excerpt of this host's transcript lines — a real substring, or real fragments joined by "…". Nothing to change.
- "fix_quote": use this whenever the published quote either doesn't carry the claim OR isn't word-for-word what the host said — i.e. it's paraphrased, compressed, smoothed, or stitches words the host didn't actually say in sequence. Supply newQuote: a verbatim ≤240-char excerpt copied EXACTLY from one of this host's transcript lines that proves the stance, plus newQuoteStartSec from that line's [<sec>s <Speaker>] prefix. A quote that captures the gist but isn't an exact substring is NOT good enough — replace it. Do NOT neutralize a real view just because the quoted sentence was weak; fix the quote.
...
- "drop": the name is mentioned only in passing — listed among others, cited as an EXAMPLE of an industry/category/trend, mentioned while reading a news item, used as a MACRO BAROMETER (a data point for a claim about the economy/consumer/rates/market rather than about itself, e.g. "Airbnb's demand warning shows the consumer is weak"), or with no investment claim anywhere. Enumerations are mentions, not theses ("companies like X and Y", "the whole chip complex"). Sector/theme sentiment ("AI is a tailwind for cybersecurity") is not a stance on any individual member company. But do NOT drop a named basket/sector/macro exposure when the basket/exposure itself is the host's explicit pick or trade ("I would be long CDS", "short the S&P", "my pick is enterprise SaaS", "stablecoins are my biggest business winner"). A formal-predictions answer like "my pick for best performing asset will be the Robinhood/Polymarket/PrizePicks gambling space" is a keep/fix_quote basket call, not a passive enumeration.
  NEVER DROP A DISCLOSED OWN POSITION. If the host states a CURRENT position of their own — "I'm a shareholder", "I own it", "I'm long", "I'm in", "I just bought the stock", "X% of our exposure is in …" — keep it (as the explicit_long it is), even when it's wrapped in an earnings/subs/news reaction. ("I am a shareholder and I am delighted they're fixing their subs" is a kept long, not a dropped reaction.) This protection is for CURRENT positions only — a PAST or involuntary one ("I was an investor… I got recapped out") is a post-mortem, not a live call, and may be dropped or made a view.
  DIVERSIFICATION IS NOT N PICKS. The disclosed-position protection above is for a name held with conviction — NOT for a name swept up in a "I own all of them" list. When ONE quote discloses a position spread across a LIST of names AND the speaker frames it as deliberately NOT choosing — "I'm an investor in OpenAI and Anthropic and Google and Microsoft and Nvidia, etc., and so you don't have to make the call which one wins", "I own the whole basket", "I'm long all of them, I don't know who wins" — that is a single diversified/hedge stance, NOT a conviction long on each member. DROP the individual member names: the same list quote cannot be the thesis for each one (it's an enumeration/list mention, and a shared quote is correctly rejected downstream). The only thing that may survive is the BASKET itself, as a single take, IF the speaker frames the basket as the pick. A name is only kept as its own scored long when it is singled out with conviction and its OWN distinct quote ("Nvidia is my biggest position", "the one I'd own here is X").
  NON-EQUITY ASSET TRANSACTIONS are not calls: buying/selling a sports franchise, real estate, art, collectibles, or another personal non-equity holding is a view at most ("Cuban sold the Mavericks" → not a scored exit).
  PREDICTION-MARKET & MARKET-STRUCTURE BETS are not equity positions. A wager on a prediction market (Polymarket, Kalshi), or on a market-STRUCTURE outcome — an index's composition or weight, "the Mag 7 shrinks below 30% of the S&P", "X happens by year-end" — is not a long or short of any named stock or ETF (the Mag 7 can rise in price while shrinking as a share of the index). Do NOT keep it as a selection/long/short on the named companies; set callType="view" (or drop if the names are pure enumeration).
...
- The scoreable shapes require the HOST'S OWN position, or their named own/buy/avoid answer to a positioning/predictions prompt, in their own words, present in the published (or fixed) quote, AND that quote must NAME this exposure (read intent in context — do NOT demand a literal "I'm long/short"):
  - explicit_long / explicit_short / explicit_exit: the host's own position IN THE STOCK — "I'm in / I just bought / I'd be long / I'm short / this is a short / I'd take profits / I'm out" — not "it's well positioned" or "you could buy it". Buying the company's PRODUCT (a car, device, subscription — "we bought two more Model Xs") is a consumer act, never an equity long; demote it to view (a forward thesis off that experience can still be a bull view/selection). Buying the SHARES ("I just bought LNG, that Cheniere company") is a real long.
  - selection: a named pick answering a positioning/allocation/predictions prompt. Beyond the obvious "your pick / #1 / best or worst performing asset / biggest winner or loser", this includes the contextual case: when the host answers "how would you reposition / where would you put money / what would you own or avoid here", the exposure they name to OWN/BUY is a bull selection and the one they name to AVOID/STAY-AWAY-FROM is a bear selection — first-person "I'm long/short" is NOT required; judge intent in context ("the easy one to avoid is treasuries", "you want to own equities over bonds here" are selections). But require the positioning/pick frame: free-floating market advice with no named pick ("buy the dip when the media panics"), product/consumer enthusiasm, and conduct/competitive analysis stay views.
